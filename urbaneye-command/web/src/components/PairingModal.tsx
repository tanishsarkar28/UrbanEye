...
export const PairingModal: React.FC<PairingModalProps> = ({
  ...
  const handlePairSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPin(''); setBusLabel(''); setRouteTag('');
      loadSessions();
      setTimeout(() => { onPairSuccess(); onClose(); setSuccessMessage(null); }, 1200);
    } catch (err: any) {
  ...
  /* ── Dark-only input style ── */
  const inputCls = 'bg-slate-900 border-slate-600 text-slate-100 placeholder:text-slate-500 focus:border-[#1E7F73] focus:outline-none focus:ring-1 focus:ring-[#1E7F73]/40';

  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-xl shadow-2xl max-w-lg w-full border border-slate-700 overflow-hidden">
        ...
        {/* ── Tab switcher ─────────────────────────────────── */}
        <div className="flex border-b border-slate-700 bg-slate-900/50 px-5 pt-3">
          <button
            type="button"
        ...
        <div className="p-6">
          {/* Error Banner */}
          {error && (
            <div className="mb-4 p-3 bg-red-900/30 border border-red-700/60 rounded-lg text-xs text-red-300 flex items-center space-x-2">
          ...
          {/* Success Banner */}
          {successMessage && (
            <div className="mb-4 p-3 bg-emerald-900/30 border border-emerald-700/60 rounded-lg text-xs text-emerald-300 flex items-center space-x-2">
          ...
          {activeTab === 'pair' ? (
            <div>
              {/* Info banner */}
              <div className="mb-4 p-3 bg-amber-900/25 border border-amber-700/50 rounded-lg text-xs text-amber-200 flex items-start space-x-2.5">
                <Shield className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                <div>
                  <strong className="text-amber-300">Secure PIN Handshake:</strong> The bus operator opens{' '}
                  <em>UrbanEye Mobile</em> on the mounted Android device to generate a 6-digit code. Enter it below to bind telemetry and detections to this district.
                </div>
              </div>
              ...
              <form onSubmit={handlePairSubmit} className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full text-center text-2xl font-mono tracking-widest font-bold py-2.5 px-3 border-2 rounded-lg ${inputCls}`}
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full py-2 px-3 border rounded-md ${inputCls}`}
                    required
                  />
                </div>
                ...
                <div className="pt-2 flex justify-end space-x-2.5">
                  <button
                    type="button"
                    onClick={onClose}
                    className="px-4 py-2 rounded-md border border-slate-600 text-slate-300 hover:bg-slate-700 font-semibold transition"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="px-5 py-2 rounded-md bg-[#1E7F73] hover:bg-[#186a60] disabled:bg-slate-700 disabled:text-slate-500 text-white font-bold flex items-center space-x-1.5 shadow-sm transition"
                  >
                    {loading ? (
                      <><Loader2 className="w-4 h-4 animate-spin" /><span>Validating &amp; Binding...</span></>
                    ) : (
                      <><Bus className="w-4 h-4" /><span>Confirm Pairing</span></>
                    )}
                  </button>
                </div>
              </form>
            ...
            <div className="space-y-3">
              ...
              {loadingSessions ? (
                ...
                <div className="p-6 bg-slate-900/50 border border-slate-700 rounded-lg text-center text-xs text-slate-500">
                  No active bus sensors paired in this district.
                </div>
              ) : (
                <div className="divide-y divide-slate-700 border border-slate-700 rounded-lg max-h-64 overflow-y-auto">
                  {sessions.map((s) => (
                    <div key={s.id} className="p-3 flex items-center justify-between hover:bg-slate-700/40 transition text-xs">
                      <div className="flex items-center space-x-2.5">
                        <div className="w-7 h-7 rounded-full bg-emerald-900/40 border border-emerald-700/60 flex items-center justify-center text-emerald-400 shrink-0">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 text-[11px] font-bold text-red-400 hover:bg-red-900/30 border border-red-700/50 rounded flex items-center space-x-1 transition disabled:opacity-50"
                      >
                        {unpairingId === s.id
              ...
              <div className="pt-3 flex justify-end">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-1.5 rounded-md border border-slate-600 text-slate-300 hover:bg-slate-700 font-semibold text-xs transition"
                >
                  Close
                </button>
              </div>
